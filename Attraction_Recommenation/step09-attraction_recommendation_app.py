...
class Exam(QWidget, form_window):
    def __init__(self):
        ...
        # model load
        self.df_reviews = pd.read_csv('./crawling_data/drop_cleaned_merged_reviews_trip_naver.csv')
        self.Tfidf_matrix = mmread('./models/Tfidf_attraction_review.mtx').tocsr()
        self.embedding_model = Word2Vec.load('./models/word2VecModel_attraction_reviews.model')
        with open('./models/tfidf.pickle', 'rb') as f:
            self.Tfidf = pickle.load(f)

        # images
        self.lbl_list = [self.lbl_0, self.lbl_1, self.lbl_2, self.lbl_3, self.lbl_4, self.lbl_5,
                         self.lbl_6, self.lbl_7, self.lbl_8, self.lbl_9, self.lbl_10,
                         self.lbl_11, self.lbl_12, self.lbl_13, self.lbl_14, self.lbl_15, self.lbl_16]
        for lbl in self.lbl_list:
            lbl.hide()
        self.lbl_list[0].show()

        # comboBox
        self.cmb_areas.setEditable(True)
        self.cmb_areas_ledit = self.cmb_areas.lineEdit()
        self.cmb_areas_ledit.setAlignment(Qt.AlignCenter)
        self.cmb_areas_ledit.setReadOnly(True)
